...
uint8_t broadcastAddress[] = {0x40, 0x4C, 0xCA, 0x3C, 0xF4, 0x6C}; // Receiver (GCS) MAC Address
...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
    ...
    if (packetID == CommandPk && len == sizeof(CommandStruct)) {
        ...
        if (g_system_status.teensy_status.ac_state > -1) {
            if ((size_t)HWSerial1.availableForWrite() >= (len + config::AVAIL_WRITE_MARGIN)) {
...
void setup() {
    ...
    HWSerial1.begin(config::SERIAL_T_BAUD, SERIAL_8N1, -1, -1); // RX, TX pins
    serialTransfer.begin(HWSerial1, true, Serial, config::SERIAL_T_TIMEOUT_MS); // Since telemetry hz is 100, 10 ms for each packet, travel time only a.5-1.5ms, might as well go to new packet
