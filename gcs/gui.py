...
class GCSWindow(QMainWindow):
    ...
    def update_status(self, status):
        ...
        text = (str(status.teensy_status.ac_state) + "\n" 
                + str(status.esp_ac_status.esp_ac_state) + "\n" 
                + str(status.teensy_status.bno_state) + "\n" 
                + str(status.teensy_status.ism_state) + "\n"
                + str(status.teensy_status.sd_state) + "\n"
                + str(status.laptop_status.laptop_state) + "\n"
                + str(status.esp_gcs_status.esp_gcs_state))
        self.ui.states_update.setText(text)
        formated_cmds = ""
    ...
    def on_serial_toggle(self):
        if self.comms_thread.serial_enabled == True:
            ...
            self.ui.serial_conn_btn.setText("Disconnect Serial")

    def on_resend_cmd(self):
        if self.comms_thread.serial_enabled == True:
            if self.comms_thread.resend_cmd(ind=(len(self.comms_thread.previous_cmds)-1)):
                self.ui.resend_cmd_input.setText("Command Queued")
            else:
                self.ui.resend_cmd_input.setText("Invalid index.")
        else:
            self.ui.resend_cmd_input.setText("No Serial")

    def on_manual_ctrl(self):
        if self.comms_thread.serial_enabled == True:
            if self.comms_thread.manual_ctrl:
                self.ui.manual_ctrl_btn.setText("MANUAL Control")
            else:
                self.ui.manual_ctrl_btn.setText("Stop MANUAL control")
            self.comms_thread.switch_manual_ctrl()
        else:
            self.ui.manual_ctrl_btn.setText("No Serial. MANUAL Ctrl")
